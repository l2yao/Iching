...
function tu_calc(ly){ //ly是取时间的方式,xm是计算的项目
 ...
 if(ly==4) jd -=29.53;
 if(ly==6) jd +=29.53;
 if(ly==4||ly==5||ly==6) jd = XL.MS_aLon_t2( Math.floor((jd+8)/29.5306)*pi2 )*36525 - curTZ/24-dt_T(jd);
 if(ly==7) jd -=29.53;
 if(ly==9) jd +=29.53;
 if(ly==7||ly==8||ly==9) jd = XL.MS_aLon_t2( Math.floor((jd-4)/29.5306)*pi2+Math.PI )*36525 - curTZ/24-dt_T(jd);
...
//==================日食概略图=================
function tu2_cls_path(){ //清除图2及特征表
  tu2.init(Can1);
  tu2.zxx.p_cls();
  tu2.zxx.p_save();
...
function tuGL_search(fs){ //查找日食
  var i,k,r,s='', n=Cp10_an.value-0;
  var jd = JD.JD( year2Ayear(Cp10_y.value), Cp10_m.value-0, 0) - J2000;  //取屏幕时间
  jd = XL.MS_aLon_t2( int2((jd+8)/29.5306)*Math.PI*2 )*36525; //定朔
  for(i=0,k=0;i<n;i++){
   if(!GS2.fastTest(jd)) { jd += 29.5306; continue; } //排除不可能的情况，加速计算
   if(fs==0) r = GS2.feature(jd,2); //低精度计算
   if(fs==1) r = GS2.feature(jd,1); //中精度计算
   if(r.lx!='N'){
    s += '<a href="javascript:tu2_calc(1,'+r.jd+');">'+JD.JD2str(r.jdSuo+J2000).substr(0,11)+'</a>';
    s += r.lx;
    k++;
    if(k%10==0) s+='<br>';
   }
...
function tu2_calc(fs,jd0){
 ...
 if(fs==1||fs==2||fs==3||fs==4){
  ...
  if(r.lx=='N') Cp10_tz.innerHTML='无日食';
   ...
   + '偏食始：'+JD.JD2str(r.gk3[2]+J2000)+' '+(r.gk3[0]*radd).toFixed(1)+','+(r.gk3[1]*radd).toFixed(1)+'<br>'
   + '中心始：'+JD.JD2str(r.gk1[2]+J2000)+' '+(r.gk1[0]*radd).toFixed(1)+','+(r.gk1[1]*radd).toFixed(1)+'<br>'
   + (r.gk5[1]!=100 ?
     '视午食：'+JD.JD2str(r.gk5[2]+J2000)+' '+(r.gk5[0]*radd).toFixed(1)+','+(r.gk5[1]*radd).toFixed(1)+'<br>' : '')
   + '中心终：'+JD.JD2str(r.gk2[2]+J2000)+' '+(r.gk2[0]*radd).toFixed(1)+','+(r.gk2[1]*radd).toFixed(1)+'<br>'
   + '偏食终：'+JD.JD2str(r.gk4[2]+J2000)+' '+(r.gk4[0]*radd).toFixed(1)+','+(r.gk4[1]*radd).toFixed(1)+'</td>'
  ...
  if(Cp10_showJX.checked){
    ...
    tu3.init(Can3);
    tu3_buff=GS3.jieX(jd,0); //取界线
    var jb=[Cp10_x0.value/10, Cp10_y0.value/10, Cp10_dx.value/10, Cp10_dy.value/10];
    tu3.draw(tu3_buff, -tu3_buff.zxJ, tu3_buff.zxW, Cp10_eR.value-0, jb, Cp10_tylx.options.selectedIndex);
  }else{
...
function tu3_xz(xm){ //旋转图3
 ...
 tu3.init(Can3);
 if(xm==0) tu3_buff.zxJ-=0.2;
 if(xm==1) tu3_buff.zxJ+=0.2;
 if(xm==2) tu3_buff.zxW+=0.2;
 if(xm==3) tu3_buff.zxW-=0.2;
 if(xm==4); //保持不变
 var jb=[Cp10_x0.value/10, Cp10_y0.value/10, Cp10_dx.value/10, Cp10_dy.value/10];
 tu3.draw(tu3_buff, -tu3_buff.zxJ, tu3_buff.zxW, Cp10_eR.value-0, jb, Cp10_tylx.options.selectedIndex);
}
...
function getNianLi(dy){ //dy起始年份偏移数
 ...
 if(y<-4712) { alert('到底了'); return; } //检查输入值
